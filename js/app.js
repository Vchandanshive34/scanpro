...
async function lensCapture() {
  ...
  busy('Reading…');
  try {
    // A burst, not a snapshot. Handheld frames vary a lot in focus, and
    // reading a smeared one is the largest single cause of a bad result:
    // on the test scenes a blurred frame scores 42% where the sharpest of a
    // burst scores 100%.
    busyUpdate('Holding steady…', 0.1);
    const frames = await grabBurst(video, { count: 4, gapMs: 70 });
    if (!frames.length) { toast('Could not read the camera.', 'error'); return; }

    busyUpdate('Reading…', 0.3);
    const shot = await captureForLens(frames);

    const cards = [];

    const codes = await scanBarcodes(shot.canvas);
    cards.push(...codes.map(codeToCard));

    ocrEngine.onProgress = ({ stage, progress }) => busyUpdate(stage, progress);

    const langs = state.settings.ocrLangs.length ? state.settings.ocrLangs : ['eng'];
    const result = await ocrEngine.recognize(shot.canvas, langs);
    ...
    if (result.text.trim()) {
      ...
      if (conf > 0 && conf < 70) {
        toast('Low confidence — hold steadier or move closer, then read again.',
          '', 4000);
      }
    }
    ...
    if (!cards.length) {
      toast('Nothing readable found. Move closer, or steady the phone.');
      return;
    }
...
async function boot() {
  ...
  // Each controller is built independently: a screen whose markup is missing
  // — a stale cached shell, a half-finished deploy — must cost that one
  // feature, not the whole app.
  const build = (name, make) => {
    try { return make(); }
    catch (err) {
      console.error(`${name} could not be set up; continuing without it.`, err);
      failedParts.push(name);
      return null;
    }
  };
  const failedParts = [];

  camera = build('Camera', () => new CameraController({
  ...
  editor = build('Editor', () => new EditorController({
  ...
  textView = build('Text view', () => new TextViewController({ onBack: back }));

  settingsView = build('Settings', () => new SettingsController({
  ...
  try {
    ...
    console.error('Some controls could not be wired up.', err);
    failedParts.push('Controls');
  }
  ...
  if (failedParts.length) {
    toast(`${failedParts.join(' and ')} unavailable — try reloading to pick up ` +
      'the latest version.', 'error', 7000);
  }
